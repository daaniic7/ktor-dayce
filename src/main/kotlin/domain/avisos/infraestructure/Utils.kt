package domain.avisos.infraestructure
...
class Utils {
    companion object  {
        fun createBase64ToImg(img: String, idAviso: String): String?{
            ...
            if(resultado!=null){
                ...
                try {
                    ...
                    val path = "Upload/images/$idAviso"
                    val dir = File(path)
                    if (!dir.exists()){
                        dir.mkdirs()
                    }
                    if (dir.isDirectory){
                        val nameFile = idAviso+"_${SimpleDateFormat("yyyyMMdd_HHmmss").format(Date())}.$extension"
        ...
        fun deleteImage(idAviso: String, img: String): Boolean{
            try {
                val path = "${ApplicationContext.context.environment.config.property("ktor.path.images").getString()}/$idAviso"
        ...
        fun deleteDirectory(idAviso: String): Boolean{
            try {
                val path = ApplicationContext.context.environment.config.property("ktor.path.images").getString()+"/$idAviso"
                val dir = File(path)
                if (dir.exists()){
